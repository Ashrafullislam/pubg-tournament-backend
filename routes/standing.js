...
router.get("/match", async (req, res) => {
  try {
    ...
    result
      .at(0)
      .teams.forEach((team) => teams.push({ id: team._id, name: team.name }));
    teams.forEach((team) => {
      standings[result.at(0)[team.id]] = result
        .at(0)
        .teams.find((i) => i._id === team.id);
    });
...
router.get("/fragger", async (req, res) => {
  try {
    ...
    const sortedPlayers = players.sort(
      (a, b) =>
        b?.kills?.[req.query["match-id"]] ||
        0 - a?.kills?.[req.query["match-id"]] ||
        0
    );
...
router.get("/overall", async (req, res) => {
  try {
    ...
    result2.forEach((match) =>
      match.teams.forEach((team) => teams2.push(team))
    );
    ...
    const filteredTeams = teams2.map((team) => {
      ...
      const pointsArray = Object.keys(newTeam?.points).map((point) => ({
        matchId: point,
        points: newTeam?.points?.[point],
      }));
      const newPointsArray = pointsArray.filter((arr) =>
        newMatchIds.includes(arr.matchId)
      );
      newTeam._id = team._id;
      // delete newTeam.players
      newTeam.points = newPointsArray.reduce(
        (a, b) => (a.points || a) + b.points,
        0
      );
      return newTeam;
    });
    ...
    sortedTeams.forEach((team) => {
      ...
      if (
        !mergedTeams.some(
          (mergedTeam) => mergedTeam._id.toString() === team._id.toString()
        )
      )
        mergedTeams.push(team2);
      else {
        const arrIndex = mergedTeams.findIndex(
          (mergedTeam) => mergedTeam._id.toString() === team._id.toString()
        );
    ...
    const newPlayers = playersResult.map((player) => {
      ...
      newPlayer.kills = Object.keys(newPlayer.kills).reduce((a, b) => {
        return newMatches.includes(a)
          ? player.kills[a]
          : 0 || 0 + newMatches.includes(b)
          ? player.kills[b]
          : 0 || 0;
...
// get best player by stage id
router.get("/overall-topfragger", async (req, res) => {
  ...
  const result = await database
    .collection("matches")
    .aggregate([
      { $match: { "stage-id": new ObjectId(groupId) } },
      {
        $lookup: {
          from: "teams",
          localField: "teams",
    ...
    .toArray();
  result.forEach((i) => matchIds.push(i._id.toString()));
  result.forEach((p) =>
    p.teams.forEach((team) => {
      team.players.forEach((player) => {
        ...
        // players.find((i) => console.log(i._id , player._id.toString()))
        const playerKills =  (player.kills[p._id.toString()])
        // console.log(players.find((i) => i._id.toString() === player._id.toString()))
        if (players.find((i) => i._id.toString() === player._id.toString())) {

          // console.log(player);
          const playerFromArray = players.find((i) => i._id.toString() === player._id.toString())
          const newPlayer = structuredClone(player);
          // console.log(newPlayer.name, newPlayer.kills[p._id])
          // console.log(player.name, p.name,  player.kills[p._id.toString()] , playerFromArray.totalKills)
          const newKills = (player.kills[p._id.toString()] ||  0) + (playerFromArray.totalKills || 0);
          console.log(newKills)
          newPlayer._id = player._id 
          // newPlayer.totalKills = 0 
          // console.log(newPlayer.totalKills,player.kills,p._id,player.kills[p._id.toString()])
          // console.log(player.kills[p._id.toString()] || player.kills[p._id] || 0 ,newPlayer.totalKills)
          // console.log(playerKills,player.totalKills)
          newPlayer.totalKills = newKills;
          
          // console.log(newPlayer.totalKills)
          const index = players.findIndex(
            (i) => i._id.toString() === player._id.toString()
          );
          players.splice(index, 1, newPlayer);
        } else {
          const newPlayer = structuredClone(player);
          newPlayer._id = player._id 
          newPlayer.totalKills = playerKills || 0 ;
          // console.log(newPlayer.totalKills)
  ...
  res.send(players.sort((a, b) => b.totalKills - a.totalKills));
});
